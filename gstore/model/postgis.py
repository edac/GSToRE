...
from sqlalchemy import Column,  literal, select, func
...
class PersistentGisElement(GisElement):
    ...
    def __init__(self, desc):
        self.desc = desc
        self.ogr = ogr.CreateGeometryFromWkb(str(desc).decode('hex'))
...
class BinaryGisElement(GisElement, expression.Function):
    """Represents a Geometry value as expressed within application code; i.e. in wkb/ewkb format.
    ...
    def __init__(self, desc, srid=-1):
        ...
        if srid != -1:
            ...
            self.spatial_reference.ImportFromEPSG(srid)
            self.ogr = ogr.CreateGeometryFromWkb(desc, self.spatial_reference)
        ...
        self.desc = desc
        f = expression.func.decode(self.desc.encode('hex'), 'hex')
        if self.ogr.GetCoordinateDimension()>2:
            f = expression.func.geomfromewkb(f) 
        else:
            f = expression.func.geomfromwkb(f)
        # OGR does not embed SRID into the EWKB as Postgis expects
        expression.Function.__init__(self, 'setsrid', f, srid)
...
def _to_postgis(value):
    ...
    if hasattr(value, '__clause_element__'):
        ...
        return BinaryGisElement(value.ExportToWkb())
...
# Any geometry enabled ORM object mapped to a Postgis table should 
# subclass Spatial
class Spatial(object):
   
    @property
    def wkt(self):
        return self.geom.ogr.ExportToWkt()

    @property
    def wkb(self):
        return self.geom.ogr.ExportToWkb()

    @property
    def box(self):
        return self.geom.ogr.GetBoundary().GetEnvelope()

    @property
    def centroid(self):
        return self.geom.ogr.Centroid()

    def to_geojson(self, geom = None, id = None, properties = {}, as_feature = True):
        """Converts from ANY shapely geom to a GeoJSON-like geometry.
        ...
        if not geom:
            ...
            if not hasattr(self, 'geom'):
                raise Exception("No geom to convert from.")
            else:
                wkt = self.wkt
                if hasattr(self, 'gid'):
                    id = self.gid
...
#
#engine = create_engine('postgresql://renzo:changeme@localhost/rgisdemo', echo=True)
#
